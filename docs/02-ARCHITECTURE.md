...
| Freshness | Not guaranteed. A proven round is a lower bound on what Chainlink has published. Consumers must check `updatedAt` against their own staleness policy. Selective-proving within a staleness window is bounded by the feed's deviation threshold (e.g. 0.25% USDC/USD, 0.5% ETH/USD `[VERIFY]`). PegGuard is immune (existence semantics). |
